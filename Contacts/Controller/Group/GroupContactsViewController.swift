...
//  Contacts
//
//  Created by Satish Babariya on 12/1/17.
...
class GroupContactsViewController: UIViewController {
    ...
    public var group: CNGroup = CNGroup()
    ...
    fileprivate func fetchContacts() -> [CNContact] {
        ...
        if Defaults[.favouritesList].count != 0 {
            ...
            let predicate: NSPredicate = CNContact.predicateForContactsInGroup(withIdentifier: group.identifier)
            do {
...
extension GroupContactsViewController: UITableViewDataSource {
...
extension GroupContactsViewController: UITableViewDelegate {
...
extension GroupContactsViewController: SwipeTableViewCellDelegate {
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath, for orientation: SwipeActionsOrientation) -> [SwipeAction]? {
        ...
        if orientation == .left {
            ...
            let deleteAction: SwipeAction = SwipeAction(style: .destructive, title: nil) { [weak self] _, _ in
                ...
                request.removeMember(self!.diffCalculator!.rows[indexPath.row], from: self!.group)
                do {
                    try store.execute(request)
                    self!.loadData()
                } catch {
                    self?.displayBottomToast(message: "Can't able to remove contact from this group.", type: .error)
                }
            }
...
extension GroupContactsViewController {
...
extension GroupContactsViewController: CNContactViewControllerDelegate {
